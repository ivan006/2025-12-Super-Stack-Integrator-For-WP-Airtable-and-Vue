...
export default {
  ...
  methods: {
    async loadConfigs() {
      ...
      const res = await fetch(`${base}/data-acceptance/index.php?endpoint=configs-fetch`)
    ...
    async fetchSource() {
      ...
      const res = await fetch(
        `${base}/data-acceptance/index.php?endpoint=source-fetch&entity=${encodeURIComponent(this.source.entity)}&id=${encodeURIComponent(this.source.id)}`
      )
    ...
    async fetchTarget() {
      ...
      const base = import.meta.env.VITE_CACHE_BASE
      const res = await fetch(
        `${base}/data-acceptance/index.php?endpoint=target-fetch&entity=${encodeURIComponent(this.target.entity)}&id=${encodeURIComponent(this.target.id)}`
      )
