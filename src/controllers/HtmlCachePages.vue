<template>
    <q-page class="q-pa-md">
        <!-- Title -->
        <div class="text-h5 q-mb-sm">HTML Cache — Static Page Freezer</div>

        <div class="text-body2 text-grey-7 q-mb-lg">
            Freeze Vue-rendered pages into static HTML files for SEO. Pages are
            rendered locally and saved as <code>/slug/index.html</code>.
        </div>
...
export default {
    ...
    methods: {
        ...
        async loadPages() {
            try {
                const res = await fetch(
                    `${this.cacheBase()}/html-cache/pages.json`,
                );
        ...
        async loadSitemaps() {
            try {
                const res = await fetch(
                    `${this.cacheBase()}/html-cache/sitemaps.json`,
                );
                    ...
                    iframe.onload = async () => {
                        setTimeout(async () => {
                            try {
                                ...
                                this.status =
                                    "❌ Iframe access blocked (same-origin required)";
                            }
                            resolve();
                        }, 1000);
                    };
